...
class DirectoryScanner:
    ...
    def scan_paths(
        ...
        for i, folder_info in enumerate(folders_to_process, 1):
            ...
            if progress_callback:
                progress_callback(i, len(folders_to_process), f"Processing: {folder_name}")
    ...
    def _should_skip_folder(
        ...
        if self.metadata_manager.has_metadata(folder_path):
            self._log(log_callback, "info", f"Skipping existing metadata: {folder_name}")
    ...
    def _process_folder(
        ...
        success = self.metadata_manager.search_and_save_metadata(folder_path, cleaned_title)
