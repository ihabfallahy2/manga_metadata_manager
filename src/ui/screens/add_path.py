...
class AddPathScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="add-path-container"):
            ...
            self.path_input = Input(
                placeholder="C:\\manga or /home/user/manga", id="path-input"
            )
    ...
    def _handle_add_path(self) -> None:
        ...
        app = self.app
        if (
            hasattr(app, "path_manager")
            and resolved_path in app.path_manager.get_paths()
        ):
    ...
    def _validate_manga_directory(self, path: Path) -> tuple[bool, str]:
        ...
        try:
            ...
            if len(subdirs) == 0:
                return True, "Empty directory (0 subdirectories)"
            else:
    ...
    def action_validate_paths(self) -> None:
        ...
        if hasattr(self.app, "path_manager"):
            ...
            if invalid_paths:
                self._show_status(
                    f"Found {len(invalid_paths)} invalid paths", "warning"
                )
            else:
