...
class AniListService:
    ...
    def search_manga(self, title: str) -> Optional[Dict[str, Any]]:
        ...
        payload = {"query": self.MANGA_QUERY, "variables": variables}

        try:
            logger.debug(f"Searching AniList for: {title}")
            response = self.session.post(
                ANILIST_URL, json=payload, timeout=REQUEST_TIMEOUT
            )
    ...
    def _process_media_data(self, media: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tags": [
                tag.get("name") for tag in media.get("tags", []) if tag.get("name")
            ],
